...
const Login = () => {
 ...
 const handleSubmit= async (e)=>{
   ...
   if(!emailValidator(input.email)){
    ...
    await axios.post("http://localhost:5000/login",input)
   .then((res)=>{
  ...
  return (
    <div className='box'>
        ...
        {error.length>0 && (<div className='error'>{error}</div>) }
            ...
            <div className="form-group">
                <label className="showlabel"><input type={"checkbox"} onClick={changeType} id="show"/>Show Password</label>
              
            </div>
            <div className="bottom-box">
                <Link>Forgot Password?</Link>
                <button type="submit" className="form-button">
                      <Link to={`/register?redirect=${redirect}/`}>Sing-up</Link>   </button>
              </div>
